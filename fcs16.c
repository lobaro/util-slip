...
#define FCS16_INITIAL   0xffff  /* Initial FCS value */
...
bool CheckFsc16(uint8_t* data, size_t len) {
	uint16_t fcs = CalcFcs16(data, len);
	return fcs == FCS16_GOOD;
}

void Test_Fcs16() {
	uint8_t bin[7] = { 1, 2, 3, 4, 5, 0,0/*236, 34*/ }; // FCS over data containing the FCS == FCS16_GOOD

	uint16_t fcs = CalcFcs16WithInit(FCS16_INITIAL, bin, 5);
	fcs ^= 0xffff; // Complement
	bin[5] = ((uint8_t) fcs); // least significant byte first
	bin[6] = ((uint8_t) (fcs >> 8));
	...
	*/
	...
	fcs = CalcFcs16WithInit(FCS16_INITIAL, bin, 7);
	if (fcs != FCS16_GOOD) {
		Log("Expected %02x but got %02x", FCS16_GOOD, fcs);
		lobaroASSERT(false);
	}
}
